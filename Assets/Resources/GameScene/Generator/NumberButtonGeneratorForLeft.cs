...
public class NumberButtonGeneratorForLeft : MonoBehaviour
{
    // ボタンのプレファブ
    public GameObject NumberButtonPrefab;
    private GameObject[] buttons;
    //演算子ボタンがあるかどうかのブーリアン，ほかの演算子ボタンgeneratorでもこれを使用する
    public static bool exist_1buttons = false;

    void Start()
    {

    }
    public void GenerateForLeft()
    {
        ...
        if (!exist_1buttons)
        {
            ...
            //ボタン生成
            //こいつは一番左だから「(」だけでいい
            this.GenerateNumberButtonsForLeft(1, 1);

            //生成したら他のgenerator達はfalseにする
            NumberButtonGeneratorForLeft.exist_1buttons = true;
    ...
    public void GenerateNumberButtonsForLeft(int rowCount, int colCount)
    {
        ...
        */
        ...
        for (int y = 0; y < rowCount; y++)
        {
            for (int x = 0; x < colCount; x++)
            {
                ...
                // ボタンの情報を設定
                var controller = button.GetComponent<NumberButtonControllerForLeft>();
                controller.SetButtonInfosForLeft(1, number.ToString());
